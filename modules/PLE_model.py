...
class Customized_Gate_Control(nn.Module):
    ...
    def forward(self, X_tasks, X_s):
        """
        X_tasks dim: (task_num, batch, embedding_dim*n_graph)
        """
        ...
        #Shared_expert_out
        Shared_expert_out = self.Shared_experts(X_s) #(batch, embedding_dim*n_graph)
        ...
        #Integrate_out
        Integrate_outs = [] #(Tasknum, batch, embedding_dim*n_graph)
        for gate, specific_expert in zip(Gate_task_out, Specific_expert_outs):
            gate = gate.unsqueeze(2) #(batch_size, 2, 1)
            specific_shared = torch.stack([specific_expert, Shared_expert_out], dim = 1) #(batch_size, 2, embedding_dim*n_graph)
            task_out = torch.matmul(specific_shared.transpose(1,2), gate)
            task_out = task_out.squeeze(2) #(batch_size, embedding_dim*n_graph)
...
class PLE(nn.Module):
    ...
    def forward(self , x):
        """
        x dim: (batch, embedding_dim*n_graph)
        """
